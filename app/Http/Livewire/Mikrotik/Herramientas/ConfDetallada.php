...
class ConfDetallada extends Component
{
    ...
    public $queue = [];
    public $isProcessing = false; // Nueva bandera para bloquear UI
        ...
        try {
            ...
            if ($res->successful()) {
                ...
                if ($status === 'ready') {
                    if ($this->activeTask) {
                        if ($data === "OK") {
                            ...
                            $this->finalizarTareaActual('error', 'Error: ' . $data);
                        }
                    } else {
                        $ifaceList = array_filter(explode('|', $data));
                        $this->interfaces = array_values($ifaceList);
                        $this->isWaitingResponse = false;
                        $this->isProcessing = false; // Fin de descubrimiento
                        $this->intentos = 0;
                    }
                    return;
                }
                ...
                if ($status === 'failed' || $status === 'error') {
                    $this->finalizarTareaActual('error', 'Socket Error');
                    return;
                }
            }
