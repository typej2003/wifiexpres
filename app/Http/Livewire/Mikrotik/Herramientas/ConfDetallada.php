...
class ConfDetallada extends Component
{
    ...
    public function iniciarDescubrimiento()
    {
        ...
        // COMANDO DE DESCUBRIMIENTO REFORZADO
        // Usamos un separador único "|" para evitar errores con nombres de interfaces con espacios
    ...
    public function checkStatus()
    {
        ...
        try {
            ...
            if ($res->successful()) {
                ...
                if ($status === 'ready') {
                    // SI HAY UNA TAREA ACTIVA (Configurando)
                    if ($this->activeTask) {
                        if ($data === "OK") {
                            $this->finalizarTareaActual('success', 'Aplicado');
                        } else {
                            $this->finalizarTareaActual('error', 'Error en Router: ' . $data);
                        }
                    } 
                    // SI NO HAY TAREA ACTIVA (Descubriendo interfaces)
                    else {
                        // Limpiamos la data y explotamos por el nuevo separador "|"
                        $ifaceList = array_filter(explode('|', $data));
                        $this->interfaces = array_values($ifaceList); // Reindexar
                ...
                if ($status === 'failed' || $status === 'error') {
                    $this->finalizarTareaActual('error', 'Fallo en Bridge');
                    return;
                }
            }
