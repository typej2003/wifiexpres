...
class ConfigurarRemoto extends Component
{
    ...
    public function ejecutarConfiguracion()
    {
        ...
        $this->iniciarProceso("🚀 Provisión Universal (Restaurando Servidores y IPs)", [
            ...
            // 9. NAT Y PERFILES (Respetando original + conexiongratis)
            ...
            ['cmd' => '/ip hotspot user profile add name="conexiongratis" shared-users=1 status-autorefresh=1m rate-limit="2M/2M" comment="Portal Gratis"', 'desc' => '9d. Perfil Gratis'],

            // 10. HOTSPOTS MÚLTIPLES (Con Trial User Profile incluido)
            ['cmd' => '/ip hotspot profile add dns-name=wifi.login hotspot-address=10.0.0.1 name=hsprof1 login-by=http-chap,http-pap,trial trial-user-profile=conexiongratis', 'desc' => '10a. Profile Hotspot'],
            ['cmd' => '/ip hotspot add address-pool=pool-wifi disabled=no interface=bridge-wifi name="hotspot-wifi" profile=hsprof1', 'desc' => '10b. Hotspot WiFi'],
            ['cmd' => ':foreach i in=[/interface ethernet find where name!="ether1"] do={ :local ename [/interface ethernet get $i name]; /ip hotspot add address-pool=("pool-" . $ename) disabled=no interface=("bridge-" . $ename) name=("hotspot-" . $ename) profile=hsprof1 }', 'desc' => '10c. Hotspots en Bridges'],
            ['cmd' => '/ip hotspot user add name=admin password=admin123', 'desc' => '10d. Usuario Admin Soporte'],
            ...
            // 12. WALLED GARDEN IP (Pasarelas + Push + DNS Bypass)
