...
class ConfigurarRemoto extends Component
{
    ...
    public function ejecutarConfiguracion()
    {
        ...
        $this->iniciarProceso("🚀 Provisión Universal (Configuración Paso a Paso)", [
            // 1. USUARIO MAESTRO
            ...
            // 3. CREACIÓN DE BRIDGES E INTERNET
            ...
            ['cmd' => '/ip dns set allow-remote-requests=yes servers=8.8.8.8,8.8.4.4; /ip dhcp-client add interface=ether1 disabled=no use-peer-dns=no comment="WAN"; /ip firewall nat add action=masquerade chain=srcnat out-interface=ether1 comment="NAT-General"', 'desc' => '6. WAN y NAT'],
            
            // 7. PRE-REQUISITO: Profile de Hotspot Base (Sin trial aún)
            ['cmd' => '/ip hotspot profile add dns-name=wifi.login hotspot-address=10.0.0.1 name=hsprof1 login-by=http-chap,http-pap,trial', 'desc' => '7. Perfil Hotspot Base'],

            // 8. RUTINA: ADDRESS -> POOL -> DHCP -> HOTSPOT (WIFI)
            ['cmd' => '/ip address add address=10.0.0.1/24 interface=bridge-wifi network=10.0.0.0; /ip pool add name=pool-wifi ranges=10.0.0.10-10.0.0.250; /ip dhcp-server add address-pool=pool-wifi disabled=no interface=bridge-wifi name="srv-wifi"; /ip dhcp-server network add address=10.0.0.0/24 dns-server=8.8.8.8 gateway=10.0.0.1; /ip hotspot add address-pool=pool-wifi disabled=no interface=bridge-wifi name="hotspot-wifi" profile=hsprof1', 'desc' => '8. Hotspot WiFi'],

            // 9. RUTINA REPETITIVA PARA CADA BRIDGE ETHERNET
            ['cmd' => ':local counter 2; :foreach i in=[/interface ethernet find where name!="ether1"] do={ 
                ...
                :local rng ("192.168." . ($counter * 10) . ".10-192.168." . ($counter * 10) . ".250"); 
                :local pname ("pool-" . $ename);
                
                /ip address add address=($ipaddr . "/24") interface=$bname network=$ipnet; 
                /ip pool add name=$pname ranges=$rng; 
                /ip dhcp-server add address-pool=$pname disabled=no interface=$bname name=("srv-" . $ename); 
                /ip dhcp-server network add address=$ipnet dns-server=8.8.8.8 gateway=$ipaddr;
                /ip hotspot add address-pool=$pname disabled=no interface=$bname name=("hotspot-" . $ename) profile=hsprof1;
                
                :set counter ($counter + 1);
            }', 'desc' => '9. Rutina Address-Pool-Hotspot por Puerto'],

            // 10. CREACIÓN DE PROFILES DE USUARIO Y ASIGNACIÓN TRIAL
            ['cmd' => '/ip hotspot user profile add name="neutro" session-timeout=1s shared-users=1; 
                      ...
                      /ip hotspot user profile add name="conexiongratis" shared-users=1 status-autorefresh=1m rate-limit="2M/2M";', 'desc' => '10. Perfiles de Usuario'],
            
            ['cmd' => '/ip hotspot profile set [find name="hsprof1"] trial-user-profile="conexiongratis"', 'desc' => '11. Asignando Trial Profile'],

            // 12. WALLED GARDEN
            ['cmd' => '/ip hotspot user add name=admin password=admin123; /ip hotspot walled-garden { remove [find]; add dst-host=wifiexpres.com; add dst-host=*.wifiexpres.com; add dst-host=*.biopagobdv.com; add dst-host=*.banvenez.com; add dst-host=biopago.banvenez.com; add dst-host=fcm.googleapis.com; add dst-host=fcm-xmpp.googleapis.com; add dst-host=mtalk.google.com; add dst-host=*.push.apple.com; add dst-host=*.push.apple.com.akadns.net; add dst-host=appleid.apple.com; add dst-host=188.95.113.44 }', 'desc' => '12. Walled Garden'],
            ['cmd' => '/ip hotspot walled-garden ip { remove [find]; add dst-address=188.95.113.44; add dst-address=190.217.7.106; add dst-address=190.217.7.229; add dst-address=200.11.243.174; add dst-address=190.202.148.187; add action=accept dst-port=5228-5230 protocol=tcp; add action=accept dst-port=5223 protocol=tcp; add action=accept dst-port=53 protocol=udp; add action=accept dst-port=53 protocol=tcp }', 'desc' => '13. Walled Garden IP'],

            // 13. PORTAL Y REBOOT
            ['cmd' => '/ip hotspot profile set [find name="hsprof1"] html-directory=hotspot; /tool fetch url="'.$downloadUrl.'" dst-path="hotspot/login.html" check-certificate=no', 'desc' => '14. Descargando Portal'],
            ['cmd' => '/system reboot', 'desc' => '15. Reiniciando equipo'],
        ]);
    }
