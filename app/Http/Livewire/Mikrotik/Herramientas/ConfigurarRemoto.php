...
class ConfigurarRemoto extends Component
{
    ...
    public function ejecutarConfiguracion()
    {
        ...
        $this->iniciarProceso("🚀 Provisión Universal (DNS Google Forzados)...", [
            // 1. USUARIO MAESTRO
            ...
            // 2. LIMPIEZA TOTAL
            ['cmd' => '/ip hotspot remove [find]; /ip dhcp-server remove [find]; /ip address remove [find where interface!="ether1"]; /interface bridge port remove [find]; /interface bridge remove [find]; /ip pool remove [find]; /ip firewall nat remove [find where comment~"Hotspot" or comment~"masq"]', 'desc' => '2. Limpieza total de servicios'],

            // 3. CREACIÓN DE BRIDGES
            ['cmd' => '/interface bridge add name=bridge-wifi; :foreach i in=[/interface ethernet find where name!="ether1"] do={ /interface bridge add name=("bridge-" . [/interface ethernet get $i name]) }', 'desc' => '3. Creando puentes'],
            
            // 4. ASIGNACIÓN DE PUERTOS
            ['cmd' => ':foreach i in=[/interface ethernet find where name!="ether1"] do={ :local ename [/interface ethernet get $i name]; :local bname ("bridge-" . $ename); /interface bridge port add bridge=$bname interface=$ename }', 'desc' => '4. Vinculando puertos físicos'],
            ...
            // 6. INTERNET, DNS GOOGLE Y NAT (Forzando 8.8.8.8)
            ['cmd' => '/ip dns set allow-remote-requests=yes servers=8.8.8.8,8.8.4.4; /ip dhcp-client add interface=ether1 disabled=no use-peer-dns=no comment="WAN"; /ip firewall nat add action=masquerade chain=srcnat out-interface=ether1 comment="NAT-General"', 'desc' => '6. WAN, DNS Google (8.8.8.8) y NAT'],
            
            // 7. IPs DINÁMICAS
            ['cmd' => '/ip address add address=10.0.0.1/24 interface=bridge-wifi network=10.0.0.0; :local counter 2; :foreach i in=[/interface ethernet find where name!="ether1"] do={ :local ename [/interface ethernet get $i name]; :local bname ("bridge-" . $ename); /ip address add address=("192.168." . ($counter * 10) . ".1/24") interface=$bname network=("192.168." . ($counter * 10) . ".0"); :set counter ($counter + 1) }', 'desc' => '7. Direccionamiento IP'],
            ...
            // 9. PERFILES DE HOTSPOT
            ['cmd' => '/ip hotspot user profile add name="neutro" session-timeout=1s shared-users=1; /ip hotspot user profile add name="cortesia 20min-0" session-timeout=20m keepalive-timeout=none shared-users=1 status-autorefresh=1m', 'desc' => '9. Perfiles de Usuario'],

            // 10. SERVIDORES HOTSPOT
            ['cmd' => '/ip hotspot profile add dns-name=wifi.login hotspot-address=10.0.0.1 name=hsprof1 login-by=http-chap,http-pap,trial; /ip hotspot add address-pool=pool-wifi disabled=no interface=bridge-wifi name="hotspot-wifi" profile=hsprof1; :foreach i in=[/interface ethernet find where name!="ether1"] do={ :local ename [/interface ethernet get $i name]; :local bname ("bridge-" . $ename); /ip hotspot add address-pool=("pool-" . $ename) disabled=no interface=$bname name=("hotspot-" . $ename) profile=hsprof1 }; /ip hotspot user add name=admin password=admin123', 'desc' => '10. Levantando Hotspots'],

            // 11. WALLED GARDEN (Dominios)
            ['cmd' => '/ip hotspot walled-garden { remove [find]; add dst-host=wifiexpres.com; add dst-host=*.wifiexpres.com; add dst-host=*.biopagobdv.com; add dst-host=*.banvenez.com; add dst-host=biopago.banvenez.com; add dst-host=fcm.googleapis.com; add dst-host=fcm-xmpp.googleapis.com; add dst-host=mtalk.google.com; add dst-host=*.push.apple.com; add dst-host=*.push.apple.com.akadns.net; add dst-host=appleid.apple.com; add dst-host=188.95.113.44 }', 'desc' => '11. Walled Garden: Dominios'],

            // 12. WALLED GARDEN IP (Pasarelas y DNS Bypass)
            ['cmd' => '/ip hotspot walled-garden ip { remove [find]; add dst-address=190.217.7.106; add dst-address=190.217.7.229; add dst-address=200.11.243.174; add dst-address=190.202.148.187; add dst-address=188.95.113.44; add dst-port=3000 protocol=tcp; add dst-port=5228-5230 protocol=tcp; add dst-port=5223 protocol=tcp; add action=accept dst-port=53 protocol=udp; add action=accept dst-port=53 protocol=tcp }', 'desc' => '12. Walled Garden IP: Pasarelas y DNS'],
