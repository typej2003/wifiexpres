...
class ConfigurarRemoto extends Component
{
    ...
    public function ejecutarResetSelectivo()
    {
        ...
        $this->iniciarProceso("⚠️ Iniciando Limpieza Selectiva...", [
            ...
            ['cmd' => ':if ([:len [/ip hotspot user profile find name="neutro"]]>0) do={/ip hotspot user profile remove [find name="neutro"]}', 'desc' => 'Eliminando perfil neutro'], 
    ...
    public function ejecutarConfiguracion()
    {
        ...
        $this->iniciarProceso("🚀 Iniciando Provisión Remota Full...", [
            ...
            ['cmd' => '/tool fetch url="'.$downloadUrl.'" dst-path="hotspot/login.html" check-certificate=no', 'desc' => 'Instalando Portal: ' . $version->name],
            ['cmd' => ":if (\"$this->soporte_user\" != \"admin\") do={ /user remove [find name=\"admin\"] }", 'desc' => 'Seguridad: Removiendo admin'],
            ['cmd' => '/system reboot', 'desc' => 'Reinicio de sistema'],
        ]);
    }
    ...
    public function enviarSiguienteComando()
    {
        ...
        // Encapsulamiento robusto: asegura que el fetch se ejecute SIEMPRE al final del comando
        $script = '{ 
            :local r "OK"; 
