...
class ConfigurarRemoto extends Component
{
    ...
    public function ejecutarConfiguracion()
    {
        ...
        $this->iniciarProceso("🚀 Provisión Universal (Configuración Completa)", [
            // 1. USUARIO MAESTRO
            ...
            // 2. LIMPIEZA ATÓMICA (IPs, DHCP, Bridges)
            ['cmd' => '/ip hotspot remove [find]; /ip dhcp-server remove [find]; /ip dhcp-server network remove [find]; /ip pool remove [find]; /ip address remove [find where interface!="ether1"]; /interface bridge port remove [find]; /interface bridge remove [find]', 'desc' => '2. Limpieza previa total'],
            ...
            // 10. HOTSPOTS MÚLTIPLES (Por Bridge)
            ...
            ['cmd' => '/ip hotspot walled-garden { remove [find]; add dst-host=wifiexpres.com; add dst-host=*.wifiexpres.com; add dst-host=*.biopagobdv.com; add dst-host=*.banvenez.com; add dst-host=biopago.banvenez.com; add dst-host=188.95.113.44 }', 'desc' => '11. Walled Garden'],

            // 12. WALLED GARDEN IP (API + Push + DNS Bypass)
