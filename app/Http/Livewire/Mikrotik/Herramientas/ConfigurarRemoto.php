...
class ConfigurarRemoto extends Component
{
    ...
    public function ejecutarConfiguracion()
    {
        ...
        $this->iniciarProceso("🚀 Provisión con Autoconexión Gratis (Trial Mode)", [
            // 1. USUARIO MAESTRO
            ...
            // 4. ASIGNACIÓN PUERTOS
            ['cmd' => ':foreach i in=[/interface ethernet find where name!="ether1"] do={ :local ename [/interface ethernet get $i name]; /interface bridge port add bridge=("bridge-" . $ename) interface=$ename }', 'desc' => '4. Asignando puertos'],
            ...
            // 9. NAT Y PERFILES (NUEVO: Creación del perfil conexiongratis)
            ['cmd' => '/ip firewall nat add action=masquerade chain=srcnat out-interface=ether1 comment="NAT-General"; ' .
                    '/ip hotspot user profile add name="neutro" session-timeout=1s shared-users=1; ' .
                    '/ip hotspot user profile add name="conexiongratis" shared-users=1 status-autorefresh=1m rate-limit="2M/2M" comment="Perfil para Portal Cautivo Gratis";', 
            'desc' => '9. NAT y Perfil Gratis'],

            // 10. HOTSPOTS (NUEVO: Configuración de Trial User Profile)
            ['cmd' => '/ip hotspot profile add dns-name=wifi.login hotspot-address=10.0.0.1 name=hsprof1 login-by=http-chap,http-pap,trial trial-user-profile=conexiongratis; ' .
                    '/ip hotspot add address-pool=pool-wifi disabled=no interface=bridge-wifi name="hotspot-wifi" profile=hsprof1; ' .
                    ':foreach i in=[/interface ethernet find where name!="ether1"] do={ :local ename [/interface ethernet get $i name]; /ip hotspot add address-pool=("pool-" . $ename) disabled=no interface=("bridge-" . $ename) name=("hotspot-" . $ename) profile=hsprof1 };', 
            'desc' => '10. Hotspots con Auto-Trial'],
